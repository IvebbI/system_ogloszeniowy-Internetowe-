...
if (isset($_POST['emailfirma'])) {
    $email = $conn->real_escape_string($_POST['emailfirma']);
$userId=$_SESSION['id'];
...
if (isset($_POST['nazwafirmyy'])) {
    $nazwa_firmy = $conn->real_escape_string($_POST['nazwafirmyy']);
    $adres_firmy = $conn->real_escape_string($_POST['adresfirmy']);
    $lokalizacjaGeograficzna_firmy = $conn->real_escape_string($_POST['lokalizacjageograficznafirmy']);
    $informacje_firmy = $conn->real_escape_string($_POST['informacjefirmy']);

    $conn->begin_transaction();


    $check_query = "SELECT * FROM firma WHERE id =" . $_SESSION["firmaID"]  ;
    $check_result = $conn->query($check_query);
    if ($check_result->num_rows > 0) {
        $query_konto = "UPDATE `firma` SET `nazwa_firmy`='$nazwa_firmy',adres = '$adres_firmy', lokalizacja_geograficzna = '$lokalizacjaGeograficzna_firmy', informacje = '$informacje_firmy' WHERE id=". $_SESSION['firmaID'];
